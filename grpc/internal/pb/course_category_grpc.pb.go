...
type CategoryServiceClient interface {
	...
	CreateCategoryStreamBi(ctx context.Context, opts ...grpc.CallOption) (CategoryService_CreateCategoryStreamBiClient, error)
...
func (c *categoryServiceClient) CreateCategoryStreamBi(ctx context.Context, opts ...grpc.CallOption) (CategoryService_CreateCategoryStreamBiClient, error) {
	stream, err := c.cc.NewStream(ctx, &CategoryService_ServiceDesc.Streams[1], "/pb.CategoryService/CreateCategoryStreamBi", opts...)
	if err != nil {
		return nil, err
	}
	x := &categoryServiceCreateCategoryStreamBiClient{stream}
	return x, nil
}

type CategoryService_CreateCategoryStreamBiClient interface {
	Send(*CreateCategoryRequest) error
	Recv() (*Category, error)
	grpc.ClientStream
}

type categoryServiceCreateCategoryStreamBiClient struct {
	grpc.ClientStream
}

func (x *categoryServiceCreateCategoryStreamBiClient) Send(m *CreateCategoryRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *categoryServiceCreateCategoryStreamBiClient) Recv() (*Category, error) {
	m := new(Category)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type CategoryServiceServer interface {
	...
	CreateCategoryStreamBi(CategoryService_CreateCategoryStreamBiServer) error
...
func (UnimplementedCategoryServiceServer) CreateCategoryStreamBi(CategoryService_CreateCategoryStreamBiServer) error {
	return status.Errorf(codes.Unimplemented, "method CreateCategoryStreamBi not implemented")
}
...
func _CategoryService_CreateCategoryStreamBi_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CategoryServiceServer).CreateCategoryStreamBi(&categoryServiceCreateCategoryStreamBiServer{stream})
}

type CategoryService_CreateCategoryStreamBiServer interface {
	Send(*Category) error
	Recv() (*CreateCategoryRequest, error)
	grpc.ServerStream
}

type categoryServiceCreateCategoryStreamBiServer struct {
	grpc.ServerStream
}

func (x *categoryServiceCreateCategoryStreamBiServer) Send(m *Category) error {
	return x.ServerStream.SendMsg(m)
}

func (x *categoryServiceCreateCategoryStreamBiServer) Recv() (*CreateCategoryRequest, error) {
	m := new(CreateCategoryRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
